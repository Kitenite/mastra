...
Get workspace instructions describing the filesystem and sandbox configuration. These instructions are injected into the agent system message via `WorkspaceInstructionsProcessor` to help agents understand the execution context.
...
When providers have an `instructions` option set, the override is applied. Pass `requestContext` to enable per-request customization:
...
#### `getPathContext()` _(deprecated)_

:::caution[Deprecated]

Use `getInstructions()` instead. `getPathContext()` will be removed in a future release.
