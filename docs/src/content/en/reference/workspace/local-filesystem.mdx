...
Returns a description of how paths work in this filesystem. Injected into the agent system message via `WorkspaceInstructionsProcessor`.
...
When an `instructions` option is provided, it overrides the default output:
...
const instructions = filesystem.getInstructions({ requestContext: ctx })
```
...
type: "RequestContext",
description: "Optional request context passed through to the instructions function",
isOptional: true,
},
]}
/>
