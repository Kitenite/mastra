...
Returns a description of how this sandbox works. Injected into the agent system message via `WorkspaceInstructionsProcessor`.
...
const instructions = sandbox.getInstructions({ requestContext: ctx })
```
